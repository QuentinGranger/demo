...
def add_retailer_header(text: str) -> str:
    header = "X-POKEMON-RETAILER-POLICY:RETAILER_WATCH_ALL_FR_V2\n"
    if header in text:
        return text
    marker = "X-POKEMON-CHANGE-POLICY:UID_STABLE_HISTORY_V1\n"
    if marker not in text:
        raise RuntimeError("Pokemon change-policy header marker missing")
    return text.replace(marker, marker + header, 1)


def replace_event(text: str, uid: str, replacement: str) -> str:
    pat = re.compile(r"BEGIN:VEVENT\nUID:" + re.escape(uid) + r"\n.*?END:VEVENT\n", re.S)
    new, count = pat.subn(replacement, text, count=1)
    if count != 1:
        raise RuntimeError(f"Expected one event for {uid}, found {count}")
    return new


def insert_before_end(text: str, event: str, uid: str) -> str:
    if f"UID:{uid}\n" in text:
        return text
    if not text.endswith("END:VCALENDAR\n"):
        raise RuntimeError("Calendar missing final END:VCALENDAR")
    return text[:-len("END:VCALENDAR\n")] + event + "END:VCALENDAR\n"


def update(path: Path, *, mega=False, play=False, retailer=False):
    text = path.read_text(encoding="utf-8")
    if retailer:
        text = add_retailer_header(text)
    if mega:
        text = replace_event(text, MEGA_UID, MEGA_EVENT)
    if play:
        text = insert_before_end(text, PLAY_EVENT, PLAY_UID)
    path.write_text(text, encoding="utf-8", newline="\n")


update(CAL / "pokemon-tcg-france.ics", mega=True, retailer=True)
update(CAL / "pokemon-events-france-paris.ics", play=True)
update(CAL / "pokemon-paris.ics", mega=True, play=True, retailer=True)
print("Pokemon calendar patch applied")
